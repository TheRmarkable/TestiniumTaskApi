...
public class ClickHelper extends GetElementHelper {
    ...
    public void clickElementMultipleTimes(String jsonKey, int times) {
        if (times <= 0) {
            log.warn("The number of clicks must be greater than 0. Provided: {}", times);
            return;
        }
        for (int i = 0; i < times; i++) {
            clickElement(jsonKey, DEFAULT_WAIT, DEFAULT_SLEEP_IN_MILLIS);
            log.info("Clicked on {} - {} time(s)", jsonKey, i + 1);
        }
    }
